...
**ANSWER** Who do you get as the first result, and what was his trueshooting percentage? Patrick and his trueshooting percentage was 63.86 %.
...
```{r}
matches_2022|> filter(team == 'Maryland Terrapins, Terps' & date <= '2022-09-04') |>
  group_by(team) |>
  summarise(
    total_attempts = sum(sh_att),
    total_on_goal = sum(so_g),
...
In 2023 with all the Women's soccer games included their total on goal number was higher than in 2022 but in 2022 with only the first 5 games calcuated there on goal percentage was higher. This showed to me 2 things that they either start off their season pretty high and fall off towards the end of the season, or the team in general is not working well together and may be getting worse and needs to make changes.
