...
matches23 |>
  ...
  group_by(result) |> summarise(avg_aces = mean(aces))
...
null hypothesis: there is no differnce between the two years. 
```{r}
umd_aces23 <- matches23 |>
  filter(team == 'Maryland Terrapins, Terps') |> mutate(season=2023) #|> filter(date < '2023-09-12')
